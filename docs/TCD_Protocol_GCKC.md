...
if (frame.Length == 131 && frame[0] == 0x47 && frame[1] == 0x43 && frame[2] == 0x4B && frame[3] == 0x43)
{
    ...
    double[] tcdValues = new double[20];
    ...
    for (int i = 0; i < 20; i++)
    {
        ...
        // 计算最终真实值
        tcdValues[i] = sign * absValue;
    }
    
    // 提取桥流值
    int bridgeCurrent = frame[128]; // 偏移量 0x80
}
```
...
* **说明**: `0x0E` 为桥流设置命令字，最后一位为具体的桥流数值（如 `0x50`）。
