...
| Topic 路径 | 发送频率 | 说明 |
| :--- | :--- | :--- |
| `vocs/device/{MN}/info` | 每次开机连线时发1次，或每1小时心跳1次 | 设备静态信息（IP, ICCID, 经纬度, 固件版本等） |
| `vocs/device/{MN}/status` | 每 1 分钟 (可配置) | 硬件工况（各路温度实测值、气路 EPC 压力实测值） |
| `vocs/device/{MN}/result` | 每个分析周期结束时 (如每 2 分钟) | 色谱分析结果（NMHC, THC, CH4 及各组分浓度） |
| `vocs/device/{MN}/log` | 事件触发（受 15 秒队列限流管控） | 设备运行日志、报警事件、报错信息 |
