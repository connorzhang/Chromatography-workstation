...
# 1. 修复 audit.js
audit_js_path = r'i:\GIT\VS2022\Chromatography-workstation\src\edge\cmd\collector\static\js\views\audit.js'
    ...
    function renderTable(history) {
        ...
        reversed.forEach(snap => {
            ...
            tr.innerHTML = `
                ...
                <td>${val(snap.tempInj2)}</td>
                <td>${val(snap.tempDet1)}</td>
                <td>${val(snap.tempDet2)}</td>
                <td>${val(snap.tempDet3)}</td>
                <td>${val(snap.carrierPsi)}</td>
                <td>${val(snap.carrierSccm)}</td>
                <td>${val(snap.h2Psi)}</td>
                <td>${val(snap.h2Sccm)}</td>
                <td>${val(snap.airPsi)}</td>
                <td>${val(snap.airSccm)}</td>
