...
export default function SequenceTable() {
  ...
  const [rows, setRows] = useState<SequenceRow[]>([
    { line: 1, sample_name: 'System Suitability', method_name: 'DEF_GC.M', inj_count: 1, sample_type: 'Calibration', multiplier: 1.0, dilution: 1.0, data_file: '001F0101.D' },
    { line: 2, sample_name: 'Blank', method_name: 'DEF_GC.M', inj_count: 1, sample_type: 'Blank', multiplier: 1.0, dilution: 1.0, data_file: '002F0201.D' },
    { line: 3, sample_name: 'Sample A', method_name: 'DEF_GC.M', inj_count: 3, sample_type: 'Sample', multiplier: 1.0, dilution: 1.0, data_file: '003F0301.D' },
    { line: 4, sample_name: 'Sample B', method_name: 'DEF_GC.M', inj_count: 1, sample_type: 'Sample', multiplier: 1.0, dilution: 10.0, data_file: '004F0401.D' },
    { line: 5, sample_name: '', method_name: '', inj_count: 1, sample_type: 'Sample', multiplier: 1.0, dilution: 1.0, data_file: '' },
    { line: 6, sample_name: '', method_name: '', inj_count: 1, sample_type: 'Sample', multiplier: 1.0, dilution: 1.0, data_file: '' },
    { line: 7, sample_name: '', method_name: '', inj_count: 1, sample_type: 'Sample', multiplier: 1.0, dilution: 1.0, data_file: '' },
    { line: 8, sample_name: '', method_name: '', inj_count: 1, sample_type: 'Sample', multiplier: 1.0, dilution: 1.0, data_file: '' },
  ]);
    ...
    try {
      ...
      if (data && data.rows && data.rows.length > 0) {
        ...
        while (loadedRows.length < 8) {
          loadedRows.push({ line: loadedRows.length + 1, sample_name: '', method_name: '', inj_count: 1, sample_type: 'Sample', multiplier: 1.0, dilution: 1.0, data_file: '' });
        }
        setRows(loadedRows);
  ...
  const saveSequence = async () => {
    const validRows = rows.filter(r => r.sample_name);
    try {
      const hostname = window.location.hostname === 'localhost' ? '127.0.0.1' : window.location.hostname;
            ...
            <table className="w-full text-xs text-left border-collapse whitespace-nowrap">
              <thead className="bg-[#e0e0e0] sticky top-0 z-10 shadow-sm">
                <tr>
                  ...
                  <th className="border border-gray-400 p-1 w-20">{t('Inj Count')}</th>
              ...
              <tbody>
                {rows.map((row, index) => (
                  <tr key={index} className="hover:bg-blue-50 focus-within:bg-blue-100">
                    ...
                    <td className="border border-gray-300 p-0">
                      <input type="number" className="w-full h-full p-1 bg-transparent outline-none focus:ring-1 focus:ring-blue-500" value={row.inj_count} onChange={(e) => handleCellChange(index, 'inj_count', parseInt(e.target.value))} />
                    </td>
                    <td className="border border-gray-300 p-0">
